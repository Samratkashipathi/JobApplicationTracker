...
class JobTrackerApp {
		...
		try {
			...
			if (data.success && data.data.logged_in) {
				if (data.data.user) {
					...
					// Still allow the app to continue if user is logged in
				}
			} else {
	...
	async loadJobs() {
		try {
			...
			const seasonParam = this.selectedSeasonId
	...
	updateCurrentSeasonDisplay(data) {
		...
		const returnButton =
			!season.is_active && this.currentSeason
		...
		container.innerHTML = `
            ...
            <div style="color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 1rem;">
                ...
                ${
					season.end_date
						? `<br>Ended: ${this.formatDate(season.end_date)}`
						: ""
				}
            </div>
	...
	updateSeasonsDisplay(seasons, selectedSeasonId = null) {
		...
		container.innerHTML = seasons
			.map((season) => {
				...
				return `
                <div class="${classes}" onclick="app.selectSeason(${
					season.id
				})">
                    ...
                    ${
						isSelected
							? '<div style="font-size: 0.8rem; color: var(--primary-color); margin-top: 0.25rem;">Currently Viewing</div>'
							: ""
					}
                </div>
            `;
	...
	updateJobsTable() {
		...
		tbody.innerHTML = this.currentJobs
			.map(
				(job) => `
            <tr>
                <td>
                    <div class="company-info">
                        ...
                        <div class="company-details">
                            ...
                            ${
								job.company_website
									? `<p><a href="${job.company_website}" target="_blank">Website</a></p>`
									: ""
							}
                        </div>
                    </div>
                ...
                <td>
                    ...
                    ${
						job.source
							? `<br><small style="color: var(--text-secondary);">via ${job.source}</small>`
							: ""
					}
                </td>
                <td>
                ...
                <td>${
					job.applied_date ? this.formatDate(job.applied_date) : "N/A"
				}</td>
                <td>${
					job.applied_date
						? this.getDaysSince(job.applied_date) + " days"
						: "N/A"
				}</td>
                <td>${job.source || "N/A"}</td>
                <td>
                    <div class="action-buttons">
                        <button class="btn-icon btn-view" onclick="app.viewJobDetails(${
							job.id
						})" title="View Details">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button class="btn-icon btn-edit" onclick="app.openStatusModal(${
							job.id
						})" title="Update Status">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn-icon btn-delete" onclick="app.deleteJob(${
							job.id
						})" title="Delete">
	...
	updateStatusChart() {
		...
		this.currentJobs.forEach((job) => {
			statusCounts[job.current_status] =
				(statusCounts[job.current_status] || 0) + 1;
		});
	...
	openModal(modalId) {
		...
		if (modal) {
			...
			if (modalId === "jobModal") {
				...
				if (appliedDateInput) {
					appliedDateInput.value = new Date()
						.toISOString()
						.split("T")[0];
				}
			}
		...
		try {
			...
			if (selectedSeason) {
				...
				this.showToast(
					"success",
					"Season Switched",
					`Now viewing: ${selectedSeason.name} ${statusText}`
				);
			}
		} catch (error) {
			...
			content.innerHTML = `
                <div class="job-details">
                    <div class="job-details-header">
                        ...
                        <div class="status-badge status-${this.getStatusClass(
							job.current_status
						)}">
                            ${job.current_status}
                        </div>
                    ...
                    <div class="job-details-grid">
                        <div class="detail-group">
                            ...
                            <p>${
								job.applied_date
									? this.formatDate(job.applied_date)
									: "N/A"
							}</p>
                        </div>
                        <div class="detail-group">
                            ...
                            <p>${
								job.applied_date
									? this.getDaysSince(job.applied_date) +
									  " days"
									: "N/A"
							}</p>
                        </div>
                    ...
                    <div class="form-actions">
                        ...
                        <button class="btn btn-primary" onclick="app.openStatusModal(${
							job.id
						}); app.closeModal('jobDetailsModal');">Update Status</button>
                    </div>
                </div>
            `;
	...
	openStatusModal(jobId) {
		...
		jobInfo.innerHTML = `
            ...
            <p>Current Status: <span class="status-badge status-${this.getStatusClass(
				job.current_status
			)}">${job.current_status}</span></p>
            <p>Applied: ${
				job.applied_date ? this.formatDate(job.applied_date) : "N/A"
			}</p>
        `;
	...
	async deleteJob(jobId) {
		...
		if (
			!confirm(
				`Are you sure you want to delete the application for ${job.role} at ${job.company_name}?`
			)
		) {
			return;
		}
	...
	async applyFilters() {
		...
		try {
			const response = await this.apiCall(
				`/jobs/filter?status=${encodeURIComponent(status)}`
			);
			this.currentJobs = response.data;
			this.updateJobsTable();
		} catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
	// Only initialize the main app if we're not on the login page
	if (!window.location.pathname.includes("/login")) {
